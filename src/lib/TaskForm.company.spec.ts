...
describe('TaskForm Company Integration', () => {
  ...
  it('should render the company autocomplete field before the project field', () => {
    ...
    const companyContainer =
      companyLabel.closest('.autocomplete-container') ||
      companyLabel.parentElement;
    const projectField = projectLabel.parentElement;

    const allFields = Array.from(
      container.querySelectorAll('.field, .autocomplete-container'),
    );
  ...
  it('should include company in the saved task and upsert it in the company store', async () => {
    ...
    await fireEvent.input(screen.getByLabelText(/Título/i), {
      target: { value: 'Task with Company' },
    });
    await fireEvent.input(screen.getByLabelText(/Empresa/i), {
      target: { value: 'My Company' },
    });
    await fireEvent.input(screen.getByLabelText(/Proyecto/i), {
  ...
  it('should not call upsertCompany if company field is empty', async () => {
    ...
    await fireEvent.input(screen.getByLabelText(/Título/i), {
      target: { value: 'Task without Company' },
    });
    await fireEvent.input(screen.getByLabelText(/Proyecto/i), {
      target: { value: 'My Project' },
    });
  ...
  it('should include company in smart fill', async () => {
    ...
    await fireEvent.input(screen.getByLabelText(/Título/i), {
      target: { value: 'Smart Task' },
    });
    await fireEvent.input(screen.getByLabelText(/Empresa/i), {
      target: { value: 'Smart Company' },
    });
    await fireEvent.input(screen.getByLabelText(/Proyecto/i), {
      target: { value: 'Smart Project' },
    });
    ...
    // Set duration
    await fireEvent.input(screen.getByLabelText(/Horas/i), {
      target: { value: '2' },
    });
    ...
    await vi.waitFor(() => {
      expect(mockCompanyStore.upsertCompany).toHaveBeenCalledWith(
        'Smart Company',
      );
    });
  });
