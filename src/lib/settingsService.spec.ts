...
describe('SettingsService', () => {
  ...
  describe('exportData', () => {
    it('should export all settings to a JSON structure', async () => {
      ...
      await db.put('projects', { name: 'Project 1', lastUsedAt: new Date('2026-01-01') });
      await db.put('companies', { name: 'Company A', lastUsedAt: new Date('2026-01-01'), useCount: 10 });

      const exported = await service.exportData();

      expect(exported.version).toBe(1);
      expect(exported.config).toContainEqual({ key: 'weeklyHoursTarget', value: 38 });
      expect(exported.config).toContainEqual({ key: 'taskTypeColor:Dev', value: '#00ff00' });
      expect(exported.projects).toContainEqual({ name: 'Project 1', lastUsedAt: expect.any(Date) });
      expect(exported.companies).toContainEqual({ name: 'Company A', lastUsedAt: expect.any(Date), useCount: 10 });
    });
  });
  ...
  describe('importData', () => {
    it('should import settings from a JSON structure and overwrite existing data', async () => {
      ...
      await db.put('projects', { name: 'Old Project', lastUsedAt: new Date('2025-01-01') });
      ...
      const backupData = {
        ...
        projects: [
          { name: 'New Project', lastUsedAt: new Date('2026-04-16').toISOString() }
        ],
        companies: [
          { name: 'New Company', lastUsedAt: new Date('2026-04-16').toISOString(), useCount: 1 }
        ]
      };
    ...
    it('should throw an error if the backup version is unsupported', async () => {
      ...
      await expect(service.importData(backupData)).rejects.toThrow('Unsupported backup version: 999');
    });

    it('should throw an error if the backup data is null', async () => {
      await expect(service.importData(null as any)).rejects.toThrow('Invalid backup file');
    });

    it('should throw an error if the backup data is not an object', async () => {
      await expect(service.importData('invalid' as any)).rejects.toThrow('Invalid backup file');
    });

    it('should throw an error if config is missing', async () => {
      const backupData = { version: 1, projects: [], companies: [] };
      await expect(service.importData(backupData as any)).rejects.toThrow('Invalid backup file');
    });
