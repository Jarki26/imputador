<script lang="ts">
  ...
  async function handleUpdate() {
    if (editingCompanyName && editValue.trim() && editingCompanyName !== editValue.trim()) {
      await companyStore.updateCompany(editingCompanyName, { name: editValue.trim() });
...
<section class="company-settings">
  ...
  <div class="add-company">
    ...
    />
    <button onclick={handleAdd} class="add-btn" disabled={!newCompanyName.trim()}>
      {i18n.t('common.add')}
    </button>
  </div>
        ...
        <div class="company-item">
          {#if editingCompanyName === company.name}
            ...
            <div class="item-actions">
              <button onclick={handleUpdate} class="icon-btn save" title={i18n.t('common.save')}>
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                  <path d="M21,7L9,19L3.5,13.5L4.91,12.09L9,16.17L19.59,5.59L21,7Z" />
                </svg>
              </button>
              <button onclick={() => (editingCompanyName = null)} class="icon-btn cancel" title={i18n.t('common.cancel')}>
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
          ...
          {:else}
            ...
            <div class="item-actions">
              <button onclick={() => startEdit(company)} class="icon-btn edit" title={i18n.t('common.update')}>
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                  <path d="M20.71,7.04C21.1,6.65 21.1,6 20.71,5.63L18.37,3.29C18,2.9 17.35,2.9 16.96,3.29L15.12,5.12L18.87,8.87M3,17.25V21H6.75L17.81,9.93L14.06,6.18L3,17.25Z" />
                </svg>
              </button>
              <button onclick={() => handleDelete(company.name)} class="icon-btn delete" title={i18n.t('common.delete')}>
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
...
<style>
  ...
  .icon-btn.edit:hover { background: var(--md-sys-color-surface-container-high); }
  .icon-btn.delete:hover { background: var(--md-sys-color-error-container); color: var(--md-sys-color-error); }
  .icon-btn.save { color: var(--md-sys-color-primary); }
  .icon-btn.save:hover { background: var(--md-sys-color-primary-container); }
  .icon-btn.cancel:hover { background: var(--md-sys-color-surface-container-high); }
</style>
